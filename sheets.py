...
def _get_config_worksheet():
    ...
    try:
        ...
        ws = spreadsheet.add_worksheet(title="Config", rows=100, cols=2)
        ws.append_row(["group_id", "thread_id"], value_input_option="RAW")
        logger.info("Создан лист Config")
    return ws


def get_config():
    """Возвращает словарь {group_id: thread_id} из листа Config."""
    try:
        ...
        logger.error("Ошибка чтения Config: %s", e)
        return {}


def set_config(group_id: int, thread_id: int):
    """Сохраняет или обновляет thread_id для group_id в листе Config."""
    try:
        ws = _get_config_worksheet()
        rows = ws.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
            if row and row[0] == str(group_id):
                ws.update(f"B{i}", [[str(thread_id)]])
                logger.info("Обновлён Config: group %s → thread %s", group_id, thread_id)
                return
        ws.append_row([str(group_id), str(thread_id)], value_input_option="RAW")
        logger.info("Добавлен Config: group %s → thread %s", group_id, thread_id)
    except Exception as e:
        logger.error("Ошибка записи Config: %s", e)
